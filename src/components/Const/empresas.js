const empresas = {
  "ESF Parque Esperança III": {
    ra: "2025004",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atenção Primária à Saúde",
    info: "Unidade Estratégia Saúde da Família com atendimento multidisciplinar. Equipe composta por médica, enfermeira, técnicas de enfermagem, agentes de saúde, e apoio do NASF com educador físico, nutricionista, psicólogo e farmacêutica.",
    plano: "Atendimento em saúde da família, curativos, administração de medicamentos, coletas, visitas domiciliares, grupos de apoio e ações preventivas.",
    descricao: "Unidade de Estratégia Saúde da Família situada em Poços de Caldas, com equipe multidisciplinar voltada ao atendimento da comunidade, composta por médica, enfermeira, técnicas de enfermagem, agentes de saúde e profissionais do NASF."
  },

  "Asilo - Associação das Damas de Caridade": {
    ra: "2025005",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Instituição de Longa Permanência para Idosos (ILPI)",
    info: "Instituição histórica fundada em 1914 pela Associação das Damas de Caridade em Poços de Caldas. Desde 1998, atua como uma Instituição de Longa Permanência para Idosos (ILPI) exclusivamente para mulheres a partir de 60 anos em situação de risco e vulnerabilidade social. A entidade se mantém por meio de doações, convênios com a Prefeitura Municipal, eventos beneficentes e um corpo de voluntários, incluindo grupos religiosos e profissionais liberais.",
    plano: "Oferece um plano de cuidado integral e multidisciplinar, contando com equipe de médica, enfermeira padrão, técnicas de enfermagem, psicóloga, nutricionista, assistente social, fisioterapeuta e cuidadoras. As atividades incluem acompanhamento de saúde completo, fisioterapia, terapia ocupacional com oficinas de artesanato, atividades religiosas em capela própria e alimentação com suporte de horta local. O processo de acolhimento envolve triagem social e assinatura de contrato de prestação de serviços.",
    descricao: "Instituição de Longa Permanência para Idosos (ILPI) fundada em 1914, mantida pela Associação das Damas de Caridade. Localizada em Poços de Caldas, acolhe mulheres idosas em situação de vulnerabilidade, oferecendo cuidado multidisciplinar completo, incluindo moradia, saúde e atividades sociais."
  },

  "ESF Dom Bosco I": {
    ra: "2025006",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atenção Primária à Saúde",
    info: "A Estratégia de Saúde da Família Dom Bosco I atende a comunidade desde 1988, com foco na promoção e prevenção da saúde. Realiza atendimentos médicos e de enfermagem, coleta de exames, entrega de medicamentos, atividades de educação em saúde e visitas domiciliares. Estrutura com salão de recepção, consultórios médico e de enfermagem, sala de medicação, sala de vacina, sala de curativo, sala para agentes de saúde, cozinha e banheiros. Conta com 12 profissionais fixos e apoio do NASF (fisioterapeuta, psicólogo, nutricionista e educador físico) em dias alternados.",
    plano: "Atendimento médico e de enfermagem, coleta de exames, administração de medicamentos, vacinação, curativos, visitas domiciliares, ações educativas em saúde, acompanhamento por agentes comunitárias e suporte multidisciplinar do NASF.",
    descricao: "Unidade da Estratégia Saúde da Família localizada no bairro Dom Bosco em Poços de Caldas. Atua desde 1988 na atenção primária à saúde, com equipe multidisciplinar e suporte do NASF, promovendo ações de prevenção, promoção e assistência à saúde da comunidade."
  },

    "ESF São José": {
    ra: "2025007",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atenção Primária à Saúde",
    info: "Criada em junho de 1999, a ESF São José atende a comunidade do Jardim Centenário em Poços de Caldas, com foco na reorganização da atenção básica segundo os princípios do SUS. A equipe multiprofissional inclui médico, enfermeiro, técnico de enfermagem, agentes comunitários de saúde e, opcionalmente, equipe de saúde bucal com dentista, auxiliar e/ou técnico. Conta também com auxiliar administrativo e de limpeza. A estrutura dispõe de salas para triagem, medicação, farmácia, vacina, consultórios médico e odontológico, sala de enfermagem, curativo, almoxarifado, banheiros coletivos e privativos, cozinha e sala dos agentes comunitários.",
    plano: "Atendimento médico, de enfermagem e odontológico, vacinação, administração de medicamentos, curativos, visitas domiciliares, ações de promoção e prevenção em saúde, além da dispensação de medicamentos e atividades com os agentes comunitários de saúde.",
    descricao: "Unidade de Estratégia Saúde da Família situada no bairro Jardim Centenário, em Poços de Caldas. Criada em 1999, oferece atendimento integral em atenção primária com equipe multiprofissional, estrutura completa e ações focadas na promoção, prevenção e recuperação da saúde da população local."
  },

    "Hospital Municipal Margarita Morales": {
    ra: "2025008",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Urgência e Emergência / Atenção Primária e Secundária",
    info: "Localizado no bairro Conjunto Habitacional Dr. Pedro Afonso Junqueira, o Hospital Municipal Margarita Morales atende toda a zona sul de Poços de Caldas com pronto atendimento 24 horas. Conta com três médicos clínicos gerais por plantão, uma enfermeira responsável técnica e três enfermeiras assistenciais. Estrutura com farmácia, raio-x, sala de inalação, CME (esterilização), sala de urgência e emergência, enfermarias masculina, feminina e pediátrica, salas de medicação, triagem com classificação de risco (Manchester), sala de sutura, consultórios médicos, setor administrativo, refeitório, cozinha e áreas de repouso para funcionários. Todas as despesas são custeadas com recursos do município.",
    plano: "Atendimento de urgência e emergência, internações de curta duração, administração de medicamentos, suturas, triagem por classificação de risco, exames de raio-x, cuidados de enfermagem, dispensação de medicamentos e suporte à atenção primária e secundária de saúde da região sul.",
    descricao: "Hospital municipal localizado na zona sul de Poços de Caldas, com funcionamento 24 horas. Oferece atendimento de urgência e emergência, internações e serviços de apoio diagnóstico e terapêutico, sendo mantido exclusivamente com recursos próprios do município."
  },
  "Irmandade do Hospital da Santa Casa de Poços de Caldas": {
    ra: "2025009",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atendimento Hospitalar de Média e Alta Complexidade",
    info: "Inaugurada em fevereiro de 1904, a Santa Casa de Poços de Caldas é um hospital referência na região, com estrutura de cinco andares e serviços de média e alta complexidade. Conta com CTI adulto e neonatal/pediátrico, centro obstétrico, pediatria, maternidade, centro cirúrgico, pronto socorro, oncologia com salão de quimioterapia, laboratório, raio-x, farmácia, CME, SND, SAME, setor de compras, lavanderia, almoxarifado, manutenção, refeitório e estacionamento. Integra o patrimônio histórico da cidade desde 1962.",
    plano: "Atendimento hospitalar em diversas especialidades médicas, internações clínicas e cirúrgicas, pronto socorro, cuidados intensivos adulto e neonatal, oncologia, obstetrícia, pediatria, realização de exames laboratoriais e de imagem, e suporte em serviços de esterilização e nutrição hospitalar.",
    descricao: "Hospital filantrópico localizado em Poços de Caldas, com mais de um século de história. Oferece assistência médica em diferentes níveis de complexidade, com foco em qualidade, segurança, humanização e responsabilidade social, atendendo pacientes da cidade e região."
  },

    "UBS Regional Sul": {
    ra: "2025010",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atenção Primária à Saúde",
    info: "Localizada na Av. Antônio Marinoni, Jardim São Sebastião, a UBS Regional Sul abrange os bairros COHAB I, COHAB II e São Sebastião. Conta com equipe multidisciplinar composta por três médicos, três enfermeiros, seis técnicos de enfermagem, três dentistas, três auxiliares de dentista, nutricionista, psicóloga, fisioterapeuta, setor administrativo, agentes comunitários de saúde e equipe de higienização. Estrutura com farmácia, consultórios odontológicos e médicos, salas de vacinação, coleta de sangue, curativos, psicologia, esterilização, reuniões, almoxarifado, cozinha, lavanderia e banheiros.",
    plano: "Realização de consultas com equipe multidisciplinar, aferição de sinais vitais, controle de glicemia, antropometria, curativos, retirada de pontos, administração de medicamentos, vacinação, dispensação de medicamentos, exames preventivos, testes rápidos de ISTs, grupos de apoio e visitas domiciliares.",
    descricao: "Unidade Básica de Saúde localizada na zona sul de Poços de Caldas, com estrutura completa e equipe multidisciplinar. Atua na atenção primária à saúde com foco na prevenção, promoção e recuperação da saúde, atendendo moradores dos bairros COHAB I, COHAB II e São Sebastião."
  },

  "UBS Regional Leste": {
    ra: "2025010",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atenção Primária à Saúde",
    info: "Localizada na Av. Antônio Marinoni, Jardim São Sebastião, a UBS Regional Sul abrange os bairros COHAB I, COHAB II e São Sebastião. Conta com equipe multidisciplinar composta por três médicos, três enfermeiros, seis técnicos de enfermagem, três dentistas, três auxiliares de dentista, nutricionista, psicóloga, fisioterapeuta, setor administrativo, agentes comunitários de saúde e equipe de higienização. Estrutura com farmácia, consultórios odontológicos e médicos, salas de vacinação, coleta de sangue, curativos, psicologia, esterilização, reuniões, almoxarifado, cozinha, lavanderia e banheiros.",
    plano: "Realização de consultas com equipe multidisciplinar, aferição de sinais vitais, controle de glicemia, antropometria, curativos, retirada de pontos, administração de medicamentos, vacinação, dispensação de medicamentos, exames preventivos, testes rápidos de ISTs, grupos de apoio e visitas domiciliares.",
    descricao: "Unidade Básica de Saúde localizada na zona sul de Poços de Caldas, com estrutura completa e equipe multidisciplinar. Atua na atenção primária à saúde com foco na prevenção, promoção e recuperação da saúde, atendendo moradores dos bairros COHAB I, COHAB II e São Sebastião."
  },
    "UPA - Unidade de Pronto Atendimento": {
    ra: "2025011",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Urgência e Emergência / Atenção Secundária",
    info: "Inaugurada em 28 de julho de 2014, a UPA de Poços de Caldas atua na atenção de urgência e emergência, funcionando 24 horas por dia. Com 1980 m², é um serviço de complexidade intermediária entre a atenção básica e a rede hospitalar. Conta com recepção, salas de classificação de risco (Protocolo de Manchester), consultórios médicos e de ortopedia, sala vermelha, laboratório, radiologia, CME, setor de observação, pediatria, sala de ECG/USG, sutura, medicação, inalação, epidemiologia, administração, nutrição, refeitório, almoxarifado, posto policial e estacionamento.",
    plano: "Atendimento de urgência e emergência, triagem com classificação de risco, consultas médicas, atendimento ortopédico, suturas, administração de medicamentos, exames laboratoriais e de imagem, eletrocardiograma, ultrassonografia, suporte em saúde pública (epidemiologia), e encaminhamentos para a rede hospitalar quando necessário.",
    descricao: "Unidade de Pronto Atendimento localizada em Poços de Caldas, com capacidade para resolver até 85% dos casos de urgência da população. Atua como referência intermediária entre as UBS/ESF e os hospitais, oferecendo atendimento médico ininterrupto e estrutura completa para diagnóstico e estabilização de pacientes."
  },

  "Hospital Municipal Vereador Gilberto de Mattos": {
    ra: "2025012",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Atendimento Ambulatorial Especializado / Atenção Secundária",
    info: "Localizado na zona leste de Poços de Caldas, o Hospital Municipal Vereador Gilberto de Mattos foi inaugurado em setembro de 2004 e revitalizado em dezembro de 2018. Atende Poços de Caldas e municípios da região como Bandeira do Sul, Andradas, Caldas e Santa Rita de Caldas. Conta com cerca de 12 especialidades médicas, incluindo Dermatologia, Neurologia, Psiquiatria, Pediatria, Urologia, Ginecologia, Mastologia, Neonatologia, Cirurgia Geral, Cabeça e Pescoço, PTGI e Pré-Natal de Alto Risco. Oferece serviços como planejamento familiar, pequenas cirurgias, exames de imagem, eletrocardiograma, fisioterapia, fonoaudiologia e cerca de 4000 atendimentos mensais.",
    plano: "Atendimento ambulatorial especializado, consultas médicas em múltiplas especialidades, realização de pequenas cirurgias, procedimentos ginecológicos, planejamento familiar, testes laboratoriais e de imagem, fisioterapia, fonoaudiologia e pré-natal de alto risco.",
    descricao: "Hospital público localizado na zona leste de Poços de Caldas, com foco em atendimento ambulatorial especializado. Atua na atenção secundária, oferecendo consultas médicas, procedimentos cirúrgicos de pequeno porte, exames e acompanhamento pré-natal, atendendo tanto a população local quanto municípios vizinhos."
  },

  
  "Local Teste 1": {
    ra: "2025013",
    polo: "Poços de Caldas",
    periodo: "",
    anoLetivo: "",
    setor: "Teste",
    info: "Teste",
    plano: "Teste",
    descricao: "Teste"
  }


  

}


export default empresas;
